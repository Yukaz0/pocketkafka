...
// lz4BlockChecksum verifies the trailing xxHash32 of a Kafka LZ4 block (used by
// some clients; we recompute and compare when the size is present).
func lz4BlockChecksum(data []byte) uint32 {
	// xxHash32 with seed 0 (simplified for verification purposes).
	...
	if len(data) >= 16 {
		...
		h = (v1<<1 | v1>>31) + (v2<<7 | v2>>25) + (v3<<12 | v3>>20) + (v4<<18 | v4>>14)
		data = data[n:]
	} else {
		h = 0x9E3779B1 + 5
	}
	for i := 0; i < len(data); i++ {
		h = xxRound(h, uint32(data[i]))
		h = (h<<11 | h>>21) * 0x9E3779B1
	}
	h ^= h >> 15
	h *= 0x85EBCA77
	h ^= h >> 13
...
func xxRound(acc, input uint32) uint32 {
	acc += input * 0xC2B2AE3D
	acc = (acc<<13 | acc>>19) * 0x27D4EB2F
	return acc
}
